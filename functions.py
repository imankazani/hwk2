...
def _read_csv_with_fallback(path, **kwargs):
    """
    Read a CSV trying UTF-8 first, then falling back to a more permissive encoding.
    Keeps everything else (skiprows, names, dtype, na_values, etc.) exactly the same.
    """
    try:
        return pd.read_csv(path, encoding="utf-8", **kwargs)
    except UnicodeDecodeError:
        # latin-1 never throws UnicodeDecodeError and is commonly needed for CMS exports
...
def read_contract(path):
    ...
    df = _read_csv_with_fallback(path, skiprows=1, names=col_names, dtype={
...
def read_enroll(path):
    ...
    df = _read_csv_with_fallback(path, skiprows=1, names=col_names, na_values="*", dtype={
